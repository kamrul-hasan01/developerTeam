...
const Team = () => {
  ...
  useEffect(() => {
    fetch("https://kamrul-hasan01.github.io/api/userDtails.json")
      .then((res) => res.json())
      .then((data) => {
        setMembers(data);
      });
        ...
        <div className="row g-0">
          ...
          <div className="col-md-4">
            <div className="col-md-12  custom-style  bg-light ms-5 sticky-top">
              <h2 className="shadow-lg p-2 rounded bg-white">
                List for Meeting
              </h2>
              <h4>
                {userIcon}Total Member :{" "}
                <span className="text-info">{meeting.length}</span>{" "}
              </h4>
              <h4>
                {taskIcon} Total Project :{" "}
                <span className="text-info">{totalProject}</span>{" "}
              </h4>

              <div className="row px-4 d-flex jusify-content-center">
                {meeting.map((meet) => (
                  <MeetingMembers meet={meet}></MeetingMembers>
                ))}
              </div>
              <button
                type="button"
                className="btn btn-outline-info bg-info text-white w-50 rounded"
              >
                {meetingButtonIcon} <span className="mx-1">Start Meeting</span>{" "}
              </button>
            </div>
          </div>
        </div>
